...
export default function Navbar() {
      ...
      return (
            <>
                  {/* Navbar */}
                  <nav className="fixed top-0 inset-x-0 w-full h-auto bg-white/70 border-b border-slate-200 backdrop-blur-sm z-50 drop-shadow-md lg:drop-shadow-none">
                        <div className="w-full lg:max-w-[1200px] md:max-w-[696px] max-w-none mx-auto lg:px-0 px-5 lg:py-4 py-5 flex items-center justify-between">
                              <div className="flex items-center space-x-[14px]">
                                    <div onClick={toggleMenu} className="w-max h-max block lg:hidden">
                                          <IconHamburger className="w-6 h-6 text-zinc-700" />
                                    </div>
                                    <Link href="/">
                                          <Image
                  ...
                  {/* Sliding Menu */}
                  <div
                        className={`fixed lg:hidden top-0 inset-y-0 left-0 w-[80%] h-full bg-white z-40 pt-[118px] transform transition-transform duration-300 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'
                              }`}
                  >
